"""Prompt templates for the DeepRepro agent system."""
...
Task: Handle paper according to input type and save to "./deeprepro_code/task{PAPER_ID}/{PAPER_ID}.md"
...
- Save path MUST be: ./deeprepro_code/task{PAPER_ID}/{PAPER_ID}.md

Processing Rules:
1. URL Input (input_type = "url"):
   - Use download_file_to tool with: url=<url>, destination="./deeprepro_code/task{PAPER_ID}/", filename="{PAPER_ID}.md"
   - Extract metadata (title, authors, year)
   - Return saved file path and metadata

2. File Input (input_type = "file"):
   - Use move_file_to tool with: source=<file_path>, destination="./deeprepro_code/task{PAPER_ID}/{PAPER_ID}.md"
...
Output Format (MANDATORY - EXACT FORMAT):
{
    ...
    "paper_path": "./deeprepro_code/task{PAPER_ID}/{PAPER_ID}.md (or null for text input)",
    "metadata": {
...
Example: If PAPER_ID=14, then paper_path should be "./deeprepro_code/task14/14.md"
"""
...
- Select exactly 5 repositories in `selected_references` when at least 5 valid candidates exist
- Do NOT list extra GitHub repository URLs outside `selected_references`
- Do NOT invent or guess repository URLs
- If a repository cannot be verified, mark it as "unverified" and provide the best candidate URL anyway
...
- The downstream downloader will use only `selected_references` and will not backfill failed downloads with unselected candidates
...
5. Select and rank exactly the top 5 references
6. Output only the final JSON object; do not include preliminary candidate lists or explanatory text with extra GitHub URLs
...
# INPUT
You receive two exhaustive analyses and optional user supplementary context:
...
3. **User Supplementary Context**: Optional user notes, constraints, preferences, or analyzed auxiliary images

Plus you can use segmented reading to access any specific paper sections needed for planning.

## User Supplementary Context Rules
- Treat user supplementary context as planning guidance, constraints, or clarification.
- Use it to refine file structure, implementation priorities, interfaces, validation targets, and details that are not obvious from text alone.
- If supplementary context conflicts with the paper, the paper text is authoritative unless the user explicitly requests a deliberate deviation.
- If no supplementary context is provided, ignore this input category.
...
The later implementation executor will primarily rely on this initial plan and may NOT re-read the full paper. Therefore, this plan must be a sufficiently detailed source of truth for the whole reproduction. Do not assume details are obvious or safe to omit. Capture all reproduction-relevant details from the main text, appendix, addendum, algorithms, equations, tables, figures, captions, experiments, datasets, prompts, and user-supplied context whenever available. A short plan can directly reduce final code quality.
...
**Total Target**: 10000-14000 characters when the paper has enough detail; prefer complete technical coverage over brevity while still avoiding filler.
...
## 4. RECORD REPRODUCTION-SPECIFIC DETAILS

This section is especially important because later subplans and file implementations will use the initial plan as their main source of paper details. Capture practical reproduction details that can easily be lost during implementation:
- datasets_and_splits: exact dataset names, splits, sample counts, preprocessing, filtering, download/setup notes, and any dataset-specific evaluation split rules
- exact_prompts: prompt text, few-shot examples, system messages, templates, answer formats, scoring rubrics, and where they are specified
- hyperparameters: model, training, inference, optimization, retrieval, decoding, ablation, and seed settings
- model_generation_settings: model names, temperature, top_p, max tokens/new tokens, sampling count, seeds, API/local model assumptions, and inference-time defaults
- evaluation_protocols: metrics, tables/figures to reproduce, baselines, ablations, test commands, aggregation rules, expected outputs, and reporting conventions
- algorithms_and_equations: algorithm steps, update rules, losses, objective functions, constraints, schedules, and special cases
- implementation_tricks: normalization, clipping, masking, caching, initialization, numerical stability, data filtering, and fallback behavior
- ambiguities_and_defaults: missing or underspecified details and the default choices to use, with clear notes that they are approximations

Be factual and specific. Do not invent exact values; if a detail is missing, record it as missing and propose a reasonable default only when needed for runnable code. Do not omit a detail merely because it seems minor; minor reproduction details often determine whether later files are correct.
...
complete_reproduction_plan:
  ...
  # SECTION 2B: Reproduction-Specific Details

  # RECORD PRACTICAL DETAILS: These details guide later subplans and file implementation.
  # Include them when present in the paper, appendix, addendum, tables, figures, captions, or user context.
  # Keep this concise; the goal is to prevent important reproduction details from being lost.

  reproduction_details: |
    datasets_and_splits:
      [Exact dataset names, splits, counts, preprocessing, filtering, setup notes, and split/evaluation rules]
    exact_prompts:
      [Exact prompt templates, few-shot examples, system prompts, scoring rubrics, answer formats, or note missing/approximate prompts]
    hyperparameters:
      [Important model/training/inference/evaluation hyperparameters and source locations]
    model_generation_settings:
      [Model names, decoding settings, max tokens, sample counts, seeds, API/local assumptions]
    evaluation_protocols:
      [Metrics, tables/figures, baselines, ablations, aggregation rules, expected outputs, and reporting conventions]
    algorithms_and_equations:
      [Algorithm steps, equations, losses, update rules, constraints, schedules, and special cases]
    implementation_tricks:
      [Normalization, clipping, masking, caching, initialization, numerical stability, filtering, fallback behavior]
    ambiguities_and_defaults:
      [Missing details and chosen practical defaults, clearly marked as defaults or approximations]
...
PLANNING_IMAGE_ANALYSIS_PROMPT = """You are analyzing a user-supplied auxiliary figure for research-paper-to-code planning.

Image #{image_index}: `{image_name}`

Focus only on information that is visually supported and useful for implementation planning.

Return a concise markdown report with these exact sections:
## Figure Type
- What kind of figure this is (architecture diagram, pipeline, table, qualitative example, ablation chart, etc.)

## Key Visual Findings
- List the main visually grounded elements, labels, modules, arrows, stages, tables, metrics, or relationships.

## Reproduction-Relevant Details
- Extract concrete implementation clues only if they are clearly visible.
- Mention model components, inputs/outputs, losses, data flow, training stages, evaluation targets, figure/table names if readable.

## Planning Impact
- Explain how this figure should influence code planning.
- If the figure is not useful for implementation planning, say so explicitly.

Rules:
- Do not invent unreadable labels.
- If something is blurry or uncertain, state the uncertainty.
- Keep terminology, label names, symbol definitions, and module names consistent with the text visible in the figure.
- If the figure contains readable prompt text, instruction text, table text, or other wording that matters for reproduction, preserve the wording faithfully instead of loosely paraphrasing it.
- Treat the figure as supplementary user context; do not assume it overrides the paper text.
"""
...
ROUND_SUBPLAN_SYSTEM_PROMPT = "You create focused JSON implementation subplans that preserve reproduction details and cross-file coherence."

REFERENCE_INDEXING_EXECUTOR_GUIDANCE_TEMPLATE = """

**Optional Reference Index Tool:**
- `search_code_references` is available because this run enabled reference indexing.
- Use `indexes_path="{indexes_path}"` exactly when calling it.
- Use it only for optional inspiration from indexed related repositories; reference results are NOT the implementation standard.
- The paper, initial reproduction plan, existing code summaries, current subplan/Next Steps, and actual same-round tool results remain authoritative.
- Planner `reference_searches` are examples and hints; you may ignore, revise, combine, or replace them with your own search keywords based on the current file and actual implementation needs.
- Good usage pattern for a file when helpful: call `search_code_references` with `target_file` and concise keywords, observe the result, then call `write_file`.
"""

ROUND_SUBPLAN_PROMPT = """You are helping an implementation agent plan the next code-writing round.

Your job is ONLY to produce a practical subplan for the executor. Do not write code.

Planning intent:
- A round is not just a batch for speed; it should be a small coherent implementation unit. Prefer a smaller high-quality batch over filling the maximum count.
- Prefer grouping files that are tightly related by interfaces, data flow, shared configuration, or caller/callee dependencies.
- The goal is to let the executor implement related files in one continuous context, reducing cross-round gaps and interface drift.
- Use rationale, execution_order, file_instructions, reproduction_notes, and integration_notes to explain how the selected files should be implemented together and which paper details must survive into code.

Hard rules:
- Choose planned_files ONLY from Remaining Files.
- Plan 1-3 files per round.
- If fewer than 3 files remain, plan all remaining files when dependencies allow.
- NEVER plan more than 3 files.
- Optionally choose at most one repair_file from Implemented Files when Recent Lightweight Diagnostics or the Full Code Knowledge Base reveal a concrete existing-file bug.
- Do not use repair_file for vague quality improvement; use it only for clear bugs such as syntax errors, interface mismatches, wrong imports, package/CLI layout errors, or invalid requirements.
- The subplan is the authoritative current-round task plan, while the executor may adapt only when blocked by missing context, dependency contradiction, or tool error.
- Preserve compatibility with already implemented public interfaces.
- Treat Implemented Files and Remaining Files as the authoritative current progress state.
- Use Recent Planner Continuity only as bounded recent context, not as full history; carry forward unfinished files only if they still appear in Remaining Files.
- Treat Recent Lightweight Diagnostics as latest non-blocking hints only; if they conflict with actual implemented code or code summaries, trust the concrete code state.
- If diagnostics mention "Final product quality gate" or blocking issues, prioritize exactly one concrete repair_file and keep planned_files empty unless Remaining Files are also present.
- Provide concrete per-file instructions so the executor knows each file's purpose, scope, interfaces, dependencies, and checks.
- Derive interface_reminders from actual implemented-code summaries, not from guesses; include exact function/class signatures when they matter.
- For files that connect modules, explicitly plan information flow: inputs, outputs, shared data shapes, side effects, error/fallback behavior, and the order in which shared interfaces should be stabilized.
- When a planned file touches datasets, prompts, configs, model generation, experiments, metrics, README, or scripts, include any relevant reproduction details from the initial plan so the executor does not forget paper-specific splits, prompt formats, hyperparameters, or evaluation protocol.
- For package `__init__.py` files, keep them lightweight and export-focused; avoid eager imports of whole subpackages or optional/heavy dependencies.
- For README/setup/CLI/requirements/config files, include checks for import paths, package layout, runnable commands, avoiding standard-library packages in requirements, and keeping setup.py install_requires aligned with real third-party requirements instead of filtering them out.
- If repair_file is selected, keep the repair narrow and concrete; do not widen it into an unrelated refactor.
- If reference indexing is enabled, you may include optional reference_searches only for repair_file or planned_files. These are search suggestions for the executor, not mandatory commands.
- Do not assume what the reference indexes contain; suggest only concise keywords that may help find patterns.
- Return ONLY valid JSON. No markdown fences, no explanations outside JSON.

Required JSON schema:
{{
  "round_id": {round_id},
  "repair_file": "optional/path/to/already_implemented_file.py or empty string",
  "repair_reason": "why this already implemented file must be fixed now, or empty string",
  "repair_instructions": ["specific minimal changes/checks for the repair file"],
  "planned_files": ["path/to/file.py"],
  "rationale": "why these files should be implemented now",
  "must_read_files": ["implemented/file.py"],
  "interface_reminders": ["interfaces or imports that must stay compatible"],
  "risks_or_checks": ["simple checks the executor should keep in mind"],
  "execution_order": ["first file", "second file"],
  "file_instructions": [
    {{
      "file_path": "path/to/file.py",
      "purpose": "the file's responsibility",
      "implementation_scope": "what must be implemented in this file",
      "key_interfaces": ["exact classes/functions/constants this file should expose, including signatures when known"],
      "dependencies": ["existing or planned files this file should use, with exact called interfaces when known"],
      "reproduction_notes": ["paper-specific details this file should preserve, such as dataset split/counts, exact prompts, hyperparameters, generation settings, evaluation protocol, or ambiguity defaults"],
      "integration_notes": ["how this file should connect with other files and what data flows across that boundary"],
      "acceptance_checks": ["simple correctness checks for this file, including interface compatibility checks"]
    }}
  ],
  "reference_searches": [
    {{
      "file_path": "planned or repair file path",
      "keywords": "comma-separated search keywords",
      "rationale": "why an optional reference search may help this file"
    }}
  ],
  "round_acceptance_checks": ["checks that should hold after this round"],
  "notes_for_executor": "concise but useful implementation guidance"
}}

# Reference Indexing Context
{reference_indexing_context}

# Initial Reproduction Plan
{initial_plan}

# Full Code Knowledge Base (all previous file summaries from implement_code_summary.md)
This section contains accumulated summaries for all files implemented so far. Use it as the main source for prior code state, public interfaces, dependencies, and implementation notes.
If the same file appears multiple times because it was repaired, treat the later/latest summary as authoritative for that file.
{code_knowledge_base}

# Implemented Files
{implemented_files_text}

# Remaining Files
{unimplemented_files}

# Recent Lightweight Diagnostics
These are recent, local, non-blocking findings from the latest executor round only. Use them as hints, not as a substitute for the code knowledge base or file lists.
{recent_diagnostics}

# Recent Planner Continuity (last two rounds only, not full history)
This section contains only the previous two rounds' subplans and executor feedback.
Use it to understand recent scheduling intent, unfinished planned files, and execution drift.
For all older rounds, rely on the Full Code Knowledge Base and current file lists instead of expecting planner history here.
{planner_context}

# Current Executor Conversation Context
{current_executor_context}
"""

SUBPLAN_EXECUTOR_MESSAGE_TEMPLATE = """**Pre-Round Implementation Subplan**

Treat this subplan as the authoritative task plan for the current implementation round, while still following the original reproduction plan and existing code summaries.
...
Execution rules:
- If repair_file is non-empty, repair that already implemented file first with one complete write_file call, then continue with planned_files.
- Follow execution_order and file_instructions by default.
- Implement planned_files in THIS SAME round when feasible.
- Use one complete write_file call per file, then continue sequentially after each tool result if more planned files remain.
- Do not skip planned files unless blocked by missing context, dependency contradiction, or tool error.
- If you must deviate, briefly state why, choose only from Remaining Files, and preserve existing interfaces.
- Treat interface_reminders, key_interfaces, dependencies, and integration_notes as hard compatibility constraints unless they conflict with actual existing code.
- Treat Recent Lightweight Diagnostics as hints from the latest round only, not as full-history truth.
- If this is a final quality-gate repair round, repair only the specified repair_file and do not create new files.
- If repair_file is present, keep the fix narrow and concrete before moving on to planned_files.
- For package `__init__.py` files, keep imports lightweight and avoid eager heavy dependencies.
- Before writing integration files, align call signatures with the existing code summaries and any same-round files already written.
- Preserve each file's reproduction_notes when present; use them as reminders for paper-specific datasets, prompts, hyperparameters, generation settings, evaluation rules, and documented defaults.
- For README/setup/CLI/requirements/config files, check import paths, package layout, runnable commands, avoid adding standard-library modules as pip dependencies, and do not filter real third-party requirements out of setup.py install_requires.
- If reference_searches are present and the reference index tool is available, treat them as examples and high-priority optional search hints, not mandatory or exclusive keywords. You may ignore, revise, combine, or replace them with your own `search_code_references` keywords based on the current file and actual implementation needs.
- Use round_acceptance_checks and each file's acceptance_checks to keep the implementation coherent.{reference_indexing_guidance}"""
...
- Package initializers should stay lightweight: keep `__init__.py` export-oriented and avoid eager imports of heavy or optional subpackages.
...
- **One Step at a Time**: Execute, observe the result, then plan and execute the next step
...
Build incrementally using multiple tool calls. For each step:
1. **Identify** what needs to be implemented from the requirements
2. **Implement** one or more closely related components/files when they are in the current plan
3. **Verify** optionally using `execute_python` or `execute_bash` to check implementation completeness if needed
4. **Integrate** with existing components
5. **Validate** against requirement specifications

**TOOL CALLING STRATEGY**:
1. **CURRENT ROUND BATCHING**: If Next Steps or a pre-round subplan lists multiple planned files, implement them in order within the current round when context is sufficient. Use one complete `write_file` call per file.

2. **TOOL CALL DISCIPLINE**: Make at most ONE tool call in each assistant response. When implementing multiple files, write one file, observe the tool result, then continue with the next planned file in a later assistant response.

3. **TOOL EXECUTION STRATEGY**:
  - **Default development cycle**: `write_file` (implement)
  - **When optional reference-index guidance is present**: `search_code_references` may be called once before `write_file` for the current file, then implement from the paper/plan/current project rather than copying references.

...
class CodeIndexer:
    ...
    def __init__(
        ...
        self.max_files_per_repository = file_analysis_config.get(
            "max_files_per_repository", 24
        )
        self.fallback_files_per_repository = file_analysis_config.get(
            "fallback_files_per_repository", 16
        )
    ...
    def _extract_target_keywords(self) -> set:
        """Extract lightweight keywords from target structure for fallback ranking."""
        keywords = set()
        if not self.target_structure:
            return keywords

        for token in re.findall(r"[A-Za-z][A-Za-z0-9_]{2,}", self.target_structure):
    ...
    def _score_file_for_fallback(self, file_path: Path, repo_path: Path) -> tuple:
        """Score a repository file without LLM calls for bounded fallback indexing."""
        try:
        ...
        if target_keywords:
            score += sum(3 for keyword in target_keywords if keyword in normalized)

        path_parts = set(re.split(r"[/_.\-\s]+", normalized))
        important_terms = {
            "src",
            "core",
            "model",
        ...
        score += sum(2 for term in important_terms if term in path_parts)
        ...
        if suffix == ".py":
            ...
            score += 3
        elif name in {"readme.md", "requirements.txt", "setup.py"}:
            score += 2

        if name.startswith("test_") or "/tests/" in f"/{normalized}":
            score -= 2
        if any(part in path_parts for part in {"docs", "examples", "notebooks"}):
            score -= 2
    ...
    def select_fallback_files(self, all_files: List[Path], repo_path: Path) -> List[Path]:
        """Select a bounded heuristic file subset when LLM pre-filtering fails."""
        if not all_files:
            return []

        limit = max(1, int(self.fallback_files_per_repository or 1))
        ranked_files = sorted(
            all_files,
            key=lambda file_path: self._score_file_for_fallback(
                file_path, repo_path
            ),
        )
        selected_files = ranked_files[:limit]
        self.logger.info(
            "LLM filtering failed; using bounded heuristic fallback: "
            f"{len(selected_files)} files selected from {len(all_files)} total"
        )
        return selected_files

    def limit_files_for_indexing(self, files: List[Path], repo_path: Path) -> List[Path]:
        """Apply a hard per-repository indexing cap to keep indexing cost bounded."""
        if not files:
            return []

        limit = max(1, int(self.max_files_per_repository or 1))
        if len(files) <= limit:
            return files

        limited_files = sorted(
            files,
            key=lambda file_path: self._score_file_for_fallback(
                file_path, repo_path
            ),
        ...
        self.logger.info(
            f"Limiting repository analysis to {len(limited_files)} files "
    ...
    async def analyze_file_content(self, file_path: Path) -> FileSummary:
        ...
        try:
            ...
            try:
                ...
                if not match:
                    raise ValueError(
                        "No JSON object found in LLM file analysis response"
                    )
    ...
    async def find_relationships(
        ...
        try:
            ...
            if not match:
                self.logger.warning(
                    f"No JSON relationship response for {file_summary.file_path}; skipping relationships"
                )
                return []
    ...
    async def process_repository(self, repo_path: Path) -> RepoIndex:
        ...
        if selected_file_paths:
            ...
            files_to_analyze = self.select_fallback_files(all_files, repo_path)

        files_to_analyze = self.limit_files_for_indexing(files_to_analyze, repo_path)
        ...
        repo_index = RepoIndex(
            ...
            analysis_metadata={
                ...
                "fallback_files_per_repository": self.fallback_files_per_repository,
...
async def main():
    ...
    config_file = "tools/indexer_config.yaml"
    api_config_file = "mcp_agent.secrets.yaml"

    # You can override these parameters or let them be read from config
    code_base_path = "deeprepro_code/task1/code_base/"  # Will use config file value if None
    output_dir = (
        "deeprepro_code/task1/indexes/"  # Will use config file value if None
    )

...
def _resolve_safe_path(working_directory: str, raw_path: str) -> Path:
    """Resolve a command path under the working directory."""
    ...
    if resolved != base_dir and base_dir not in resolved.parents:
        raise ValueError(f"Refusing to operate outside working directory: {raw_path}")
    return resolved


def _handle_portable_file_command(
    command: str, working_directory: str
) -> Optional[str]:
    """Handle common Unix file-tree commands without depending on the host shell."""
    try:
    ...
    if executable == "mkdir":
        paths = [part for part in parts[1:] if part not in {"-p", "--parents"}]
        if not paths:
            return None
        for raw_path in paths:
            _resolve_safe_path(working_directory, raw_path).mkdir(
                parents=True, exist_ok=True
            )
        return f"created directories: {', '.join(paths)}"
    ...
    if executable == "touch":
        ...
        if not paths:
            return None
        for raw_path in paths:
            file_path = _resolve_safe_path(working_directory, raw_path)
            file_path.parent.mkdir(parents=True, exist_ok=True)
            file_path.touch(exist_ok=True)
        return f"created files: {', '.join(paths)}"
...
async def execute_command_batch(
    ...
    try:
        ...
        for i, command in enumerate(command_lines, 1):
            try:
                portable_result = _handle_portable_file_command(
                    command, working_directory
                )
                if portable_result is not None:
                    results.append(f"✅ Command {i}: {command}")
                    results.append(f"   输出 / Output: {portable_result}")
                    stats["successful"] += 1
                    continue
...
async def execute_single_command(
    ...
    try:
        ...
        portable_result = _handle_portable_file_command(command, working_directory)
        if portable_result is not None:
            return [
                types.TextContent(
                    type="text",
                    text=format_portable_command_result(
                        command, working_directory, portable_result
                    ),
                )
            ]
...
def format_portable_command_result(
    command: str, working_directory: str, portable_result: str
) -> str:
    return f"""

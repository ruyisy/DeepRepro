...
class DialogueLogger:
    ...
    def __init__(self, paper_id: str, base_path: str = None):
        ...
        Args:
            ...
            base_path: Base path for logs (defaults to the DeepRepro workspace)
        """
        self.paper_id = paper_id
        self.base_path = (
            base_path
            or os.path.join(os.getcwd(), "deeprepro_code")
        )
        self.log_directory = os.path.join(
            self.base_path, f"task{paper_id}", "logs"
        )
...
def extract_paper_id_from_path(path: str) -> str:
    ...
    Returns:
        Paper ID string
    """
    # Extract task ID from path like "/data2/.../deeprepro_code/task1/initial_plan.txt"

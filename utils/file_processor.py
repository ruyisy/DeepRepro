...
class FileProcessor:
    ...
    def extract_file_path(file_info: Union[str, Dict]) -> Optional[str]:
        ...
        try:
            ...
            paper_path = info_dict.get("paper_path") or info_dict.get("file_path")
            if not paper_path:
                raise ValueError("No paper_path or file_path found in input dictionary")
    ...
    async def process_file_input(
        ...
        """
        try:
            # 棣栧厛灏濊瘯浠庡瓧绗︿覆涓彁鍙杕arkdown鏂囦欢璺緞
            ...
            # If base_dir is provided, keep task directories inside that workspace.
            if base_dir and paper_dir:
                base_dir = os.path.abspath(base_dir)
                paper_dir = os.path.abspath(paper_dir)
                try:
                    is_inside_base = os.path.commonpath([base_dir, paper_dir]) == base_dir
                except ValueError:
                    is_inside_base = False

                if is_inside_base:
                    pass
                elif paper_dir.endswith("deeprepro_code"):
                    paper_dir = base_dir
                else:
                    paper_name = os.path.basename(paper_dir)
                    paper_dir = os.path.join(base_dir, paper_name)
            ...
            file_path = None
            if isinstance(file_input, str):
                # 灏濊瘯瑙ｆ瀽涓篔SON锛堝鐞嗕笅杞界粨鏋滐級
                try:
                    parsed_json = json.loads(file_input)
                    if isinstance(parsed_json, dict) and (
                        "paper_path" in parsed_json or "file_path" in parsed_json
                    ):
                        file_path = parsed_json.get("paper_path") or parsed_json.get(
                            "file_path"
                        )
                        # 濡傛灉鏂囦欢涓嶅瓨鍦紝灏濊瘯鏌ユ壘markdown鏂囦欢
                        ...
                        raise ValueError(
                            "Invalid JSON format: missing paper_path or file_path"
                        )
                except json.JSONDecodeError:
                    extracted_json = cls.extract_json_from_text(file_input)
                    if extracted_json and (
                        "paper_path" in extracted_json or "file_path" in extracted_json
                    ):
                        file_path = extracted_json.get(
                            "paper_path"
                        ) or extracted_json.get("file_path")
                        # 濡傛灉鏂囦欢涓嶅瓨鍦紝灏濊瘯鏌ユ壘markdown鏂囦欢
                        if file_path and not os.path.exists(file_path):
                            paper_dir = os.path.dirname(file_path)
                            if os.path.isdir(paper_dir):
                ...
                # Dictionary input
                file_path = file_input.get("paper_path") or file_input.get("file_path")
        ...
        for match in matches:
            try:
                ...
                if isinstance(parsed, dict) and (
                    "paper_path" in parsed or "file_path" in parsed
                ):
                    return parsed
            except json.JSONDecodeError:
                continue

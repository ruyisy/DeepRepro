...
- Reference-aware reproduction analysis with configurable execution modes
...
from workflows.agents.planning_image_agent import (
    analyze_planning_images,
    copy_planning_images_to_paper_dir,
    save_planning_image_analysis,
)
from utils.workspace_paths import get_task_dir, get_workspace_dir, next_task_id
...
def get_default_search_server(config_path: str = "mcp_agent.config.yaml"):
    ...
    try:
        if os.path.exists(config_path):
            ...
            print(f"🔍 DeepRepro search provider: {default_server}")
            return default_server
        else:
            print(f"⚠️ Config file {config_path} not found; using DeepRepro default search provider: brave")
            return "brave"
    except Exception as e:
        print(f"⚠️ Error reading config file {config_path}: {e}")
        print("🔍 Falling back to DeepRepro default search provider: brave")
        return "brave"
...
async def run_research_analyzer(prompt_text: str, logger) -> str:
    ...
    try:
        ...
        print("📊 DeepRepro is analyzing the research paper...")
...
async def run_resource_processor(analysis_result: str, logger) -> str:
    ...
    """
    # Pre-compute task ID - deterministic, no LLM needed
    workspace_dir = get_workspace_dir()
    os.makedirs(workspace_dir, exist_ok=True)
    next_id = next_task_id(workspace_dir)
    paper_dir = get_task_dir(next_id)
    os.makedirs(paper_dir, exist_ok=True)

    logger.info(f"📋 DeepRepro task ID: {next_id}")
    logger.info(f"📂 DeepRepro task directory: {paper_dir}")
...
async def run_code_analyzer(
    ...
    Args:
        ...
        supplementary_requirements: Optional user-supplied planning notes or constraints
        planning_image_summary: Optional summary generated from user-supplied planning images
    ...
    search_attempt_timeout_seconds = 600

    def build_agent_config() -> Dict[str, list]:
        base_agent_config = get_adaptive_agent_config(
            use_segmentation, search_server_names
        )
        ...
        return {
            "concept_analysis": base_agent_config["concept_analysis"],
            "algorithm_analysis": search_server_names + ["filesystem"],
            "code_planner": search_server_names + ["filesystem"],
        }

    def build_code_aggregator() -> Tuple[ParallelLLM, Dict[str, list]]:
    ...
    def build_request_params(current_max_tokens: int, current_temperature: float) -> RequestParams:
        return RequestParams(
    ...
    if supplementary_requirements.strip():
        supplementary_context_parts.append(
            "=== USER-SUPPLIED REQUIREMENTS START ===\n"
            f"{supplementary_requirements.strip()}\n"
            "=== USER-SUPPLIED REQUIREMENTS END ==="
        )
    if planning_image_summary.strip():
        supplementary_context_parts.append(
            "=== USER-SUPPLIED AUXILIARY FIGURE ANALYSIS START ===\n"
            f"{planning_image_summary.strip()}\n"
            "=== USER-SUPPLIED AUXILIARY FIGURE ANALYSIS END ==="
        )

    supplementary_context_block = ""
    if supplementary_context_parts:
        supplementary_context_block = (
            "\n\n=== USER SUPPLEMENTARY CONTEXT START ===\n"
            + "\n\n".join(supplementary_context_parts)
            + "\n=== USER SUPPLEMENTARY CONTEXT END ===\n\n"
            "Use the user supplementary context only as planning guidance, constraints, or clarification.\n"
            "If there is any conflict, the paper text takes priority unless the user explicitly requests a deliberate deviation.\n"
        )

    def build_planning_message() -> str:
        search_instruction_block = (
        ...
        if paper_content:
            ...
            return f"""Analyze the research paper provided below. The paper file has been pre-loaded for you.
...
The goal is to create a reproduction plan detailed enough for independent implementation."""
            ...
            return f"""Analyze the research paper in directory: {paper_dir}
{supplementary_context_block}
...
The goal is to create a reproduction plan detailed enough for independent implementation."""
    ...
    max_retries = 3
    retry_count = 0
    current_max_tokens = max_tokens_limit
    current_temperature = temperature
    result = ""

    while retry_count < max_retries:
        try:
            code_aggregator_agent, _ = build_code_aggregator()
            enhanced_params = build_request_params(
                current_max_tokens, current_temperature
            )
            ...
            result = await asyncio.wait_for(
                ...
                timeout=search_attempt_timeout_seconds,
            )
            ...
            print(f"Code analysis timed out on attempt {retry_count + 1}")
            retry_count += 1
    ...
    if not result or not result.strip():
        raise ValueError("Code planning failed: LLM returned empty result after retries")
...
async def github_repo_download(search_result: str, paper_dir: str, logger) -> str:
    ...
    Returns:
        str: Download result
    """
    from tools.git_command import download_github_repo

    target_dir = os.path.join(paper_dir, "code_base")
    instruction = (
        f"{search_result}\n\n"
        f"TARGET_DIR: {target_dir}\n"
        "Download all selected GitHub repositories to the TARGET_DIR above."
    )
    return await download_github_repo(instruction)


async def verify_reference_github_urls(reference_result: str, logger) -> str:
    """Verify and lightly correct GitHub URLs found by the reference analyzer."""
    from tools.git_command import verify_and_correct_github_urls

    verified_result, report = await verify_and_correct_github_urls(reference_result)
    if report:
        logger.info(f"GitHub URL verification completed: {'; '.join(report)}")
...
Goal: Find the most valuable GitHub repositories from the paper's reference list for code implementation reference."""
    ...
    async with reference_analysis_agent:
        ...
        return await verify_reference_github_urls(reference_result, logger)
...
async def synthesize_workspace_infrastructure_agent(
    ...
    return {
        ...
        "final_report_path": os.path.join(paper_dir, "final_report.md"),
        "implementation_report_path": os.path.join(
            paper_dir, "code_implementation_report.txt"
        ),
        "implementation_result_path": os.path.join(
            paper_dir, "code_implementation_result.json"
        ),
...
def _json_safe_default(value: Any) -> str:
    """Fallback serializer for workflow artifacts."""
    return str(value)


def _extract_code_implementation_report(implementation_result: Any) -> str:
    """Extract the human-readable final implementation report from workflow result."""
    if isinstance(implementation_result, dict):
        results = implementation_result.get("results", {})
        if isinstance(results, dict):
            report = results.get("code_implementation")
            if isinstance(report, str) and report.strip():
                return report
            if report is not None:
                return json.dumps(report, ensure_ascii=False, indent=2, default=_json_safe_default)

        status = implementation_result.get("status", "unknown")
        message = (
            implementation_result.get("message")
            or implementation_result.get("error")
            or "No code implementation report was returned."
        )
        return f"# Code Implementation Report\n\n- Status: {status}\n- Message: {message}\n"

    if isinstance(implementation_result, str):
...
def _save_code_implementation_outputs(
    implementation_result: Dict[str, Any],
    dir_info: Dict[str, str],
    event_callback: Optional[Callable] = None,
) -> Dict[str, str]:
    """
    Persist implementation outputs in stable, separately readable artifacts.

    `final_report.md` and `code_implementation_report.txt` contain the actual
    final report text. `code_implementation_result.json` preserves the structured
    wrapper returned by the workflow for debugging and frontend consumption.
    """
    report_text = _extract_code_implementation_report(implementation_result)
    report_paths = {
        "final_report_path": dir_info["final_report_path"],
        "implementation_report_path": dir_info["implementation_report_path"],
        "implementation_result_path": dir_info["implementation_result_path"],
    }

    os.makedirs(dir_info["paper_dir"], exist_ok=True)
    for path_key in ("final_report_path", "implementation_report_path"):
        with open(report_paths[path_key], "w", encoding="utf-8") as f:
            f.write(report_text)

    with open(report_paths["implementation_result_path"], "w", encoding="utf-8") as f:
        json.dump(
            implementation_result,
            f,
            ensure_ascii=False,
    ...
    if event_callback:
        try:
            event_callback(
                "artifact",
                {
                    "artifact_type": "implementation_outputs",
                    "final_report_path": report_paths["final_report_path"],
                    "implementation_report_path": report_paths[
                        "implementation_report_path"
                    ],
                    "implementation_result_path": report_paths[
...
async def orchestrate_reference_intelligence_agent(
    ...
    if os.path.exists(reference_path):
        ...
        verified_reference_result = await verify_reference_github_urls(
            reference_result, logger
        )
        if verified_reference_result != reference_result:
            with open(reference_path, "w", encoding="utf-8") as f:
                f.write(verified_reference_result)
            print(f"Verified reference analysis saved to {reference_path}")
        return verified_reference_result
    ...
    with open(reference_path, "w", encoding="utf-8") as f:
        ...
        print(f"DeepRepro reference analysis saved to {reference_path}")
...
async def orchestrate_code_planning_agent(
    ...
    # Check if initial plan already exists and is non-empty
    if not os.path.exists(initial_plan_path) or os.path.getsize(initial_plan_path) == 0:
        ...
        supplementary_requirements = dir_info.get("supplementary_requirements", "")
        planning_image_summary = dir_info.get("planning_image_summary", "")
        initial_plan_result = await run_code_analyzer(
            dir_info["paper_dir"],
            logger,
            use_segmentation=use_segmentation,
            supplementary_requirements=supplementary_requirements,
            planning_image_summary=planning_image_summary,
        )
...
async def orchestrate_codebase_intelligence_agent(
    ...
    try:
        ...
        print(f"DeepRepro reference indexing failed: {e}")
        print("Continuing with implementation without blocking on the reference index...")
...
async def synthesize_code_implementation_agent(
    ...
    if event_callback:
        event_callback(
            "stage",
            {
                ...
                "message": "DeepRepro is entering the implementation loop.",
            },
        )
    ...
    try:
        # Use one unified implementation workflow; indexing only enables reference tools.
        if enable_indexing:
            print(
                f"Starting DeepRepro implementation loop ({implementation_mode} mode, reference indexing enabled)..."
            )
        else:
            print(
                f"Starting DeepRepro implementation loop ({implementation_mode} mode, reference indexing disabled)..."
            )
        code_workflow = CodeImplementationWorkflow(
            implementation_mode=implementation_mode,
            enable_reference_indexing=enable_indexing,
            event_callback=event_callback,
        )

        # Check if initial plan file exists and is non-empty
        if os.path.exists(dir_info["initial_plan_path"]) and os.path.getsize(
            dir_info["initial_plan_path"]
        ) > 0:
            ...
            report_paths = _save_code_implementation_outputs(
                implementation_result, dir_info, event_callback
            )

            # Log implementation results
            if implementation_result["status"] == "success":
                print("DeepRepro implementation completed successfully!")
                print(f"Generated workspace: {implementation_result['code_directory']}")
                print(
                    f"DeepRepro final report saved to {report_paths['final_report_path']}"
                )
                print(
                    "Structured DeepRepro implementation result saved to "
                    f"{report_paths['implementation_result_path']}"
                )

            else:
                print(
                    f"DeepRepro implementation failed: {implementation_result.get('message', 'Unknown error')}"
                )

            if isinstance(implementation_result, dict):
                implementation_result["final_report_path"] = report_paths[
                    "final_report_path"
                ]
                implementation_result["implementation_report_path"] = report_paths[
                    "implementation_report_path"
                ]
                implementation_result["implementation_result_path"] = report_paths[
                    "implementation_result_path"
                ]
        ...
        print(f"DeepRepro implementation workflow failed: {e}")
...
async def execute_multi_agent_research_pipeline(
    ...
    Args:
        ...
        implementation_mode: Code execution strategy, fast or deepplan
        supplementary_requirements: Optional user-supplied planning notes or constraints
    ...
    try:
        ...
        if event_callback:
            event_callback(
                "stage",
                {
                    "stage": "workspace",
                    "label": "Workspace",
                    "message": "Preparing the DeepRepro workspace.",
                    "implementation_mode": implementation_mode,
                    "reference_indexing": enable_indexing,
                },
            )

        print("🚀 Initializing DeepRepro multi-agent reproduction harness")

        # Setup local workspace directory
        workspace_dir = get_workspace_dir()
        os.makedirs(workspace_dir, exist_ok=True)

        print("📁 DeepRepro environment: local")
        print(f"📂 DeepRepro workspace directory: {workspace_dir}")
        print("✅ Workspace status: ready")

        # Log intelligence functionality status
        if enable_indexing:
            print("🧠 Reference indexing enabled - DeepRepro will build searchable code memory")
        else:
            print("⚡ Reference indexing disabled - DeepRepro will run the direct implementation path")
        ...
        if event_callback:
            event_callback(
                "stage",
                {
                    ...
                    "message": "Analyzing the paper and preparing source text.",
                },
            )
        ...
        dir_info["planning_image_paths"] = []
        dir_info["planning_image_summary"] = ""
        dir_info["planning_image_analysis_path"] = None
        dir_info["supplementary_requirements"] = supplementary_requirements or ""

        if planning_image_paths:
            print("DeepRepro is processing user-supplied planning images...")
            try:
                copied_planning_image_paths = copy_planning_images_to_paper_dir(
                    dir_info["paper_dir"], planning_image_paths, logger
                )
                dir_info["planning_image_paths"] = copied_planning_image_paths

                if copied_planning_image_paths:
                    planning_image_result = await analyze_planning_images(
                        copied_planning_image_paths,
                        logger=logger,
                    )
                    dir_info["planning_image_summary"] = planning_image_result.get(
                        "combined_summary", ""
                    )
                    dir_info["planning_image_analysis_path"] = (
                        save_planning_image_analysis(
                            dir_info["paper_dir"], planning_image_result, logger
                        )
                    )
                    print(
                        f"   Planning image analysis status: {planning_image_result.get('status', 'unknown')}"
                    )
                else:
                    print("   No valid planning images were copied into the workspace")
            except Exception as planning_image_error:
                print(
                    f"Planning image analysis skipped due to error: {planning_image_error}"
                )
        ...
        if event_callback:
            event_callback(
                "stage",
                {
                    "stage": "planning",
                    "label": "Planning",
                    "message": "Synthesizing the reproduction plan and target file tree.",
                },
            )
        ...
        if enable_indexing:
            if event_callback:
                event_callback(
                    "stage",
                    {
                        "stage": "reference_indexing",
                        "label": "Reference & Indexing",
                        "message": "Finding reference repositories and building searchable indexes.",
                    },
                )
            ...
            print(
                f"🔶 Skipping reference intelligence analysis ({implementation_mode} mode, indexing disabled)"
            )
            # Create empty reference analysis result to maintain file structure consistency
            reference_result = (
                "Reference intelligence analysis skipped - indexing disabled for "
        ...
        if enable_indexing:
            ...
            print(
                f"🔶 Skipping automated repository acquisition ({implementation_mode} mode, indexing disabled)"
            )
            # Create empty download result file to maintain file structure consistency
            with open(dir_info["download_path"], "w", encoding="utf-8") as f:
                f.write(
                    "Automated repository acquisition skipped - indexing disabled for "
                    f"{implementation_mode} mode"
                )
        ...
        if enable_indexing:
            ...
            print(
                f"🔶 Skipping codebase intelligence orchestration ({implementation_mode} mode, indexing disabled)"
            )
            ...
            index_result = {
                ...
                "message": (
                    "Codebase intelligence orchestration skipped for "
                    f"{implementation_mode} mode"
                ),
            }
            with open(dir_info["index_report_path"], "w", encoding="utf-8") as f:
        ...
        if enable_indexing:
            ...
            pipeline_summary = (
                f"Multi-agent research pipeline completed ({implementation_mode} mode, indexing disabled) "
                f"for {dir_info['paper_dir']}"
            )

        # Add indexing status to summary
        if not enable_indexing:
            pipeline_summary += (
                f"\n⚡ {implementation_mode} mode: GitHub download and codebase indexing skipped"
            )
        ...
        if implementation_result["status"] == "success":
            ...
            if implementation_result.get("final_report_path"):
                pipeline_summary += (
                    "\n📝 Final report saved in: "
                    f"{implementation_result['final_report_path']}"
                )
        ...
        print(f"DeepRepro pipeline failed: {e}")
        raise e
...
async def execute_chat_based_planning_pipeline(
    ...
    try:
        ...
        chat_paper_dir = os.path.join(workspace_dir, paper_name)

...
class CodeImplementationAgent:
            ...
            try:
                ...
                if self.mcp_agent:
                    ...
                    if tool_name == "write_file":
                        self._track_file_implementation_with_summary(
                            tool_call, result
                        )
    ...
    def _track_file_implementation_with_summary(
                ...
                if file_path and file_content:
                    summary_task = asyncio.create_task(
                        self._create_summary_task(
                            ...
                            self.memory_agent.current_round,
                        )
                    )
                    self.pending_summary_tasks.append(summary_task)
                    summary_task.add_done_callback(self._discard_summary_task)
                    self.logger.info(f"Queued async code summary for: {file_path}")
                else:
                    self.logger.warning(
    ...
    async def _create_summary_task(
        ...
        """Create and persist one file summary in the background."""
    ...
    def _discard_summary_task(self, task: asyncio.Task):
        """Remove completed summary tasks and log failed background summaries."""
        if task in self.pending_summary_tasks:
            self.pending_summary_tasks.remove(task)
        try:
            task.result()
        except Exception as e:
            self.logger.error(f"Async code summary task failed: {e}")

    async def wait_for_pending_summaries(self):
        """Wait until all queued file summaries are persisted."""
        if not self.pending_summary_tasks:
            return

        pending_tasks = list(self.pending_summary_tasks)
        self.logger.info(
            f"Waiting for {len(pending_tasks)} pending code summary task(s)"
        )

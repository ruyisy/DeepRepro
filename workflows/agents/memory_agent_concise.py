...
2. During an outer implementation round: preserve executor conversation continuity
3. After the outer round: compact to initial plan + accumulated code summaries + file lists

Key Features:
- Preserves system prompt and initial plan always
- Stores per-file summaries in implement_code_summary.md
- Tracks current round tool results from essential tools:
  * read_code_mem, read_file, write_file
  * execute_python, execute_bash
  * search_code, search_reference_code/search_code_references, get_file_structure
- Provides clean, focused input for the next outer implementation round
"""
...
class ConciseMemoryAgent:
    ...
    - Use a clean slate after each outer round, not between files in the same round
    - Preserve accumulated per-file summaries for cross-file continuity
    - Remove conversational clutter before the next outer round

    Essential Tools Tracked:
    - File Operations: read_code_mem, read_file, write_file
    - Code Analysis: search_code, search_reference_code/search_code_references, get_file_structure
    ...
    def __init__(
        ...
        reference_indexing_enabled: bool = False,
        reference_indexes_path: str = "",
    ):
        """
        ...
        Args:
            ...
            reference_indexing_enabled: Whether search_code_references is available
            reference_indexes_path: Absolute indexes directory path for search_code_references
        """
        self.logger = logger or self._create_default_logger()
        self.initial_plan = initial_plan_content
        self.reference_indexing_enabled = (
            reference_indexing_enabled and bool(reference_indexes_path)
            and os.path.isdir(reference_indexes_path)
        )
        self.reference_indexes_path = reference_indexes_path.replace("\\", "/")
        ...
        # Memory state tracking - trigger once the outer workflow decides to compact.
        self.last_write_file_detected = (
            False  # Track if write_file was called in current iteration
        )
        self.should_clear_memory_next = False  # Flag to compact executor messages.
        ...
        if target_directory:
            ...
            self.save_path = "./deeprepro_code/task1/"
        ...
        self.implementation_progress_path = os.path.join(
            self.save_path, "implementation_progress.json"
        )
        self.subplans_dir = os.path.join(self.save_path, "subplans")

        # Current round tool results storage
        self.current_round_tool_results = []
        self._summary_write_lock = asyncio.Lock()
        ...
        self.logger.info(
            "Memory optimization is controlled by the outer implementation loop"
        )
    ...
    def record_file_implementation(
        ...
        if file_path and file_path not in self.all_files_list:
            self.all_files_list.append(file_path)
            self.all_files_list = sorted(self.all_files_list)
    ...
    async def create_code_implementation_summary(
        ...
        try:
            ...
            summary_round = self.current_round if round_id is None else round_id
            ...
            await self._save_code_summary_to_file(
                formatted_summary, file_path, summary_round
            )
            ...
            summary_round = self.current_round if round_id is None else round_id
            fallback_summary = self._create_fallback_code_summary(
                file_path,
                implementation_content,
                files_implemented,
                summary_round,
            ...
            await self._save_code_summary_to_file(
                fallback_summary, file_path, summary_round
            )
    ...
    def _create_code_summary_prompt(
        ...
        """
        current_round = self.current_round if round_id is None else round_id
...
**CRITICAL: This is a per-file summary only. Do NOT plan future files here.**

**All Previously Implemented Files:**
{implemented_files_list}

**Remaining Unimplemented Files:**
...
- Class {{ClassName}}: {{purpose}} | Exact constructor signature: `__init__({{params}})` | Key methods: `method_name({{params}}) -> {{return_type}}`
- Function `{{function_name}}({{params}}) -> {{return_type}}`: {{purpose, inputs, outputs}}
...
- Information Flow: {{main inputs consumed, outputs returned/written, state changes, side effects, errors/fallbacks}}
- If this file is a package `__init__.py`, state clearly whether it is lightweight/export-only or performs eager imports, and list any symbols it re-exports.
- Completion Status: {{complete, partial, mock/fallback, or intentionally minimal; mention any limitation that future files must know}}

**Instructions:**
- Be precise and concise
- Focus on interfaces and information flow that other files will need
- Extract actual function signatures from the code; do not invent or simplify signatures
- Include parameter defaults, return semantics, side effects, and important data shapes when visible
- If this file defines mock/fallback behavior, incomplete API clients, placeholder data, or compatibility assumptions, state that explicitly
- Do NOT include a "Next Steps" section
- Do NOT suggest which file should be implemented next
...
**Summary:**"""
    ...
    def _format_code_implementation_summary(
        ...
        #         formatted_summary = f"""# DeepRepro Implementation Summary
        # **All Previously Implemented Files:**
        # {implemented_files_list}
        # **Generated**: {timestamp}
        # **File Implemented**: {file_path}
        ...
        # """
        formatted_summary = f"""## FILE SUMMARY
- file_path: {file_path}
- round_id: {summary_round}
- generated_at: {timestamp}
- total_files_implemented: {files_implemented}
...
"""
    ...
    def _create_fallback_code_summary(
        ...
        summary = f"""# DeepRepro Implementation Summary
**All Previously Implemented Files:**
{implemented_files_list}
...
"""
    ...
    async def _save_code_summary_to_file(
        self, new_summary: str, file_path: str, round_id: Optional[int] = None
    ):
        """
        ...
        try:
            ...
            async with self._summary_write_lock:
                ...
                with open(self.code_summary_path, "a", encoding="utf-8") as f:
                    if not file_exists:
                        ...
                        f.write("# DeepRepro Implementation Progress Summary\n")
                        f.write("*Accumulated DeepRepro implementation progress for all files*\n\n")

                    # Add clear separator between implementations
                    f.write("\n" + "=" * 80 + "\n")
                    f.write(
                        f"## IMPLEMENTATION File {file_path}; ROUND {round_id if round_id is not None else self.current_round} \n"
                    )
                    f.write("=" * 80 + "\n\n")
    ...
    def start_new_round(self, iteration: Optional[int] = None):
        ...
        if iteration is not None:
            ...
            # self.logger.info(f"馃攧 Synced round with workflow iteration {iteration}")
        else:
    ...
    def record_tool_result(
        ...
        essential_tools = [
            ...
            "search_code_references",  # Search indexed reference code (if available)
    ...
    def create_concise_messages(
        ...
        Build a compact context from the plan, file summaries, file lists, and Next Steps.
        ...
        # Use concise approach with clean slate.
        self.logger.info(
        ...
        reference_guidance = ""
        if self.reference_indexing_enabled:
            reference_guidance = f"""

**Optional Reference Index Tool:**
- `search_code_references` is available for optional inspiration from indexed related repositories.
- Use `indexes_path="{self.reference_indexes_path}"` exactly when calling the tool.
- Use it only when reference patterns can help the current file; the paper, initial plan, existing summaries, and current subplan/Next Steps remain authoritative."""
...
**Objective:** {"Reply 'All files implemented' to finish" if not unimplemented_files else "Continue implementation by analyzing dependencies and implementing the next required file(s) according to the plan's priority order and any Next Steps batch."}""",
        }
        ...
        knowledge_base_message = {
            "role": "user",
            "content": f"""**Below is the Knowledge Base of ALL implemented code files so far:**
...
**For NEW file implementation(s) (if remaining files exist):**
Use `write_file` once per file. If Next Steps lists multiple files, implement them sequentially: write one file, observe the tool result, then continue with the next planned file. Preserve compatible interfaces between them.

{"**Optional indexed references:** You may call `search_code_references` before `write_file` for a current planned file when useful, using the exact indexes_path above. Reference results are inspiration only." if self.reference_indexing_enabled else ""}
...
**Remember:** Stop and declare completion when all files are done!""",
    ...
    def _extract_latest_implementation_entry(self, content: str) -> Optional[str]:
        ...
        try:
            ...
            # self.logger.info("Extracted latest implementation entry from knowledge base")
...
{self._format_tool_result_content(tool_result)}
""")
            elif tool_name == "search_code_references":
                target_file = tool_input.get("target_file", "unknown")
                keywords = tool_input.get("keywords", "")
                indexes_path = tool_input.get("indexes_path", "")
                formatted_results.append(f"""
**search_code_references Result for {target_file} (indexes_path: {indexes_path}, keywords: {keywords}):**
{self._format_tool_result_content(tool_result)}
""")
...
""")
    ...
    def save_round_subplan(self, round_id: int, subplan: Dict[str, Any]):
        """Persist the subplan generated for one implementation round."""
        try:
            os.makedirs(self.subplans_dir, exist_ok=True)
            subplan_path = os.path.join(self.subplans_dir, f"round_{round_id:03d}.json")
            with open(subplan_path, "w", encoding="utf-8") as f:
                json.dump(subplan, f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.logger.error(f"Failed to save round subplan: {e}")

    def save_implementation_progress(self, round_id: int):
        """Persist progress snapshot from existing memory statistics."""
        try:
            progress = self.get_memory_statistics(len(self.implemented_files))
            progress.update(
                {
            ...
            os.makedirs(os.path.dirname(self.implementation_progress_path), exist_ok=True)
            with open(self.implementation_progress_path, "w", encoding="utf-8") as f:
                json.dump(progress, f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.logger.error(f"Failed to save implementation progress: {e}")

    def build_subplan_context(
        self, round_id: int, recent_diagnostics: str = ""
    ) -> Dict[str, Any]:
        """Build full context for pre-round subplanning."""
        file_lists = self.get_formatted_files_lists()
        return {
            "round_id": round_id,
            "initial_plan": self.initial_plan,
            "code_knowledge_base": self._read_code_knowledge_base() or "",
            "implemented_files": self.implemented_files.copy(),
            "unimplemented_files": self.get_unimplemented_files(),
            "implemented_files_text": file_lists["implemented"],
            "unimplemented_files_text": file_lists["unimplemented"],
            "next_steps": "",
            "recent_diagnostics": recent_diagnostics,
        }

    def create_round_next_steps_prompt(
        self,
        round_id: int,
        completed_this_round: List[str],
        diagnostics: str = "",
    ) -> str:
        """Create a prompt for fast-mode next-round file selection."""
        file_lists = self.get_formatted_files_lists()
        completed_text = (
            "\n".join(f"- {file_path}" for file_path in completed_this_round)
            if completed_this_round
            else "- No new files were completed in this round"
        )
        return f"""You are selecting the next implementation batch for a file-by-file code generation workflow.

Your job is ONLY to choose the next 1-6 files for the following implementation round. Do not write code.

Hard rules:
- Choose files ONLY from "Remaining Unimplemented Files".
- Choose 1-6 files.
- Prefer closely related files that can be implemented together in one round.
- Respect dependency order and existing public interfaces.
- NEVER choose a file from "All Previously Implemented Files".
- Return ONLY the exact Next Steps lines, no extra prose.
...
# All Previously Implemented Files
{file_lists["implemented"]}

# Remaining Unimplemented Files
{file_lists["unimplemented"]}

# Full Code Knowledge Base
{self._read_code_knowledge_base() or "No implemented code summary yet."}

# Recent Lightweight Diagnostics
{diagnostics or "None"}

# Required Output Format
- Code will be implemented: path/to/file.py
- Code will be implemented: path/to/other_file.py
"""

    async def generate_round_next_steps(
        self,
        client,
        client_type: str,
        round_id: int,
        ...
        """Generate and store fast-mode Next Steps after a batch round completes."""
        if not self.get_unimplemented_files():
            self.current_next_steps = ""
            return ""

        prompt = self.create_round_next_steps_prompt(
            round_id, completed_this_round, diagnostics
        )
        try:
            response = await self._call_llm_for_next_steps(
                client, client_type, [{"role": "user", "content": prompt}]
            )
            next_steps = (response.get("content") or "").strip()
        except Exception as e:
            self.logger.error(f"Failed to generate round Next Steps: {e}")
            next_steps = ""

        next_steps = self._normalize_round_next_steps(next_steps)
        if not next_steps:
            next_steps = "\n".join(
                f"- Code will be implemented: {file_path}"
                for file_path in self.get_unimplemented_files()[:1]
            )

        self.current_next_steps = next_steps
        self.logger.info(
            f"Round Next Steps stored ({len(next_steps)} chars)"
        )
        return next_steps

    def _normalize_round_next_steps(self, next_steps: str) -> str:
        """Keep only valid remaining files in canonical Next Steps format."""
        remaining_files = self.get_unimplemented_files()
        selected_files = []

        for line in (next_steps or "").splitlines():
            stripped = line.strip().strip("-* ")
            if not stripped:
                continue

            marker = "Code will be implemented:"
            ...
            for remaining_file in remaining_files:
                if file_path == remaining_file or remaining_file in stripped:
                    matched_file = remaining_file
                    break

            if matched_file and matched_file not in selected_files:
    ...
    async def _call_llm_for_next_steps(
        self, client, client_type: str, messages: List[Dict]
    ) -> Dict[str, Any]:
        """Call LLM for fast-mode next-step file selection."""
        system_prompt = (
            "You select the next 1-6 unimplemented files for a code generation workflow. "
            "Return only canonical Next Steps lines and never include prose."
        )
    ...
    def get_memory_statistics(self, files_implemented: int = 0) -> Dict[str, Any]:
        ...
        return {
            ...
            "implementation_progress_percent": min(
                100,
                (
    ...
    def should_trigger_memory_optimization(
        ...
        Trigger when the outer implementation loop has recorded new file writes.
        ...
        """
        # Trigger if the workflow left the compaction flag enabled.
    ...
    def apply_memory_optimization(
        ...
        Compact executor history into plan + accumulated summaries + current file lists.
    ...
    def debug_concise_state(self, files_implemented: int = 0):
        ...
        print("")
        print(
            "Memory cycle: outer round -> compact context -> next outer round"
        )

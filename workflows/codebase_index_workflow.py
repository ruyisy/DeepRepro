...
class CodebaseIndexWorkflow:
    ...
    def _looks_like_file_tree(self, text: str) -> bool:
        """Return whether text is likely a project file tree."""
        ...
        return any(any(marker in line for marker in file_markers) for line in lines)

    def _extract_file_structure_from_yaml(self, plan_content: str) -> Optional[str]:
        """Extract file_structure from fenced or raw YAML initial plans."""
        yaml_blocks = re.findall(
            plan_content,
            re.DOTALL | re.IGNORECASE,
        )
        candidates = yaml_blocks + [plan_content]
        ...
        for candidate in candidates:
            ...
            root = parsed.get("complete_reproduction_plan", parsed)
            if not isinstance(root, dict):
                continue

            file_structure = root.get("file_structure")
            if isinstance(file_structure, str) and self._looks_like_file_tree(
                file_structure
            ):
    ...
    def extract_file_tree_from_plan(self, plan_content: str) -> Optional[str]:
        ...
        """
        yaml_file_tree = self._extract_file_structure_from_yaml(plan_content)
        if yaml_file_tree:
            self.logger.info(
                f"Extracted file tree structure from YAML file_structure ({len(yaml_file_tree.splitlines())} lines)"
            )
            return yaml_file_tree
        ...
        self.logger.warning(
            "No standard file tree found, trying to extract from file mentions..."
        )
        ...
        self.logger.warning("No file tree structure found in initial plan")
        return None
    ...
    def load_target_structure_from_plan(self, plan_path: str) -> str:
        ...
        try:
            ...
            if file_tree:
                ...
                file_tree_line_count = len(file_tree.split("\n"))
                if file_tree_line_count > 8:
                    self.logger.info(
                        f"   ... {file_tree_line_count - 8} more lines"
                    )
                return file_tree
            else:
                self.logger.warning("Unable to extract file tree from initial plan")
...
"""
    ...
    async def run_indexing_workflow(
        ...
        """
        try:
            self.logger.info("Starting DeepRepro reference index workflow...")
            ...
            if os.path.exists(initial_plan_path):
                ...
                self.logger.warning(
                    f"Initial plan file does not exist: {initial_plan_path}"
                )
            ...
            if "file_analysis" in indexer_config:
                ...
                self.indexer.max_files_per_repository = file_config.get(
                    "max_files_per_repository",
                    self.indexer.max_files_per_repository,
                )
                self.indexer.fallback_files_per_repository = file_config.get(
                    "fallback_files_per_repository",
                    self.indexer.fallback_files_per_repository,
                )
            ...
            self.logger.info(
                f"Content caching: {'Enabled' if self.indexer.enable_content_caching else 'Disabled'}"
            )
            self.logger.info(
            ...
            self.logger.info("=" * 60)
            self.logger.info("Building DeepRepro reference code indexes...")
            ...
            if output_files:
                ...
                if self.indexer.generate_statistics:
                    ...
                    for file_path in output_files.values():
                        try:
                            ...
                            self.logger.warning(
                                f"   Unable to load statistics from {file_path}: {e}"
                            )
                ...
                self.logger.warning("No index files generated")
                return {
                    "status": "warning",
    ...
    def print_banner(self):
        ...
        banner = """
============================================================
  DeepRepro Reference Index Workflow
  Intelligent code relationship analysis for reproduction
============================================================
"""
        print(banner)
...
async def main():
    ...
    # Test parameters
    paper_dir = "./deeprepro_code/task1"
    initial_plan_path = os.path.join(paper_dir, "initial_plan.txt")
